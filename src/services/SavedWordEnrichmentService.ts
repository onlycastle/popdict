...
export class SavedWordEnrichmentService {
  ...
  needsEnrichment(entry: SavedWordRecord, language: TargetLanguage | null): boolean {
    return entry.details === null || entry.details.translationLanguage !== language
  }
  ...
  async enrich(
    ...
    if (!this.needsEnrichment(entry, language) && entry.details) return entry.details
    ...
    if (language && canonical) {
      const fromCache = cached?.translations[language]
      translations = fromCache ?? await retry(() => this.deps.translate(canonical, language))
    }
