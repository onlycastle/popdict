-- Answer behavior is determined by the persisted quiz origin, never by a
-- caller-controlled route flag. Keep the legacy boolean argument so a schema-
-- first rollout remains compatible with the previously deployed function.

alter table public.quizzes
  add constraint quizzes_source_check check (source in ('email', 'app'));
...
comment on function public.record_quiz_answer(uuid, integer, boolean) is
  'Atomically records an answer; quiz source controls weekly streak updates. p_touch_streak is retained for rollout compatibility and ignored.';
